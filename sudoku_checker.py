...
def check_rows(user_grid):
    ...
    check_squares(user_grid, results, check_list)


def check_squares(user_grid, results, check_list):
    # This function is now working as expected do not alter!
    print(f'results received {results}')
    check_count = 0
    blocks_of_3_rows = [[user_grid[0],
                         user_grid[1],
                         user_grid[2]],
    ...
    for i in range(3):
        ...
        sq_3 = []
        for j in range(0, 3):
            sq_1.append(blocks_of_3_rows[i][0][j])
            sq_1.append(blocks_of_3_rows[i][1][j])
            sq_1.append(blocks_of_3_rows[i][2][j])

        for j in range(3, 6):
            sq_2.append(blocks_of_3_rows[i][0][j])
            sq_2.append(blocks_of_3_rows[i][1][j])
            sq_2.append(blocks_of_3_rows[i][2][j])

        for j in range(6, 9):
            sq_3.append(blocks_of_3_rows[i][0][j])
            sq_3.append(blocks_of_3_rows[i][1][j])
            sq_3.append(blocks_of_3_rows[i][2][j])
    ...
    print(f'Blocks results: {results}')
    check_columns(user_grid, results, check_list)
...
def check_columns(user_grid, results, check_list):
    ...
    if column_result == 9:
        ...
        results['columns'] = False

    print(f'column results: {results}')
